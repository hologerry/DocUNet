# import sys
# sys.path.insert(0, '/root/DocUNet')
import argparse
import os
...
if __name__ == '__main__':
    ...
    file_save_folder = '/root/DocUNet/test/' + file_basename
    if not os.path.exists(file_save_folder):
        os.mkdir(file_save_folder)
    os.system('cp ' + img_path + ' ' + file_save_folder + '/' + file_basename + '.png')
    os.system('cp ' + os.path.dirname(os.path.dirname(img_path)) + '/label/' + file_basename + '.npy' + ' ' + file_save_folder + '/' + file_basename + '.npy')
    save_path = os.path.join(file_save_folder, file_basename + '-prediction.png')
    np.savez_compressed(os.path.join(file_save_folder, file_basename + '-prediction.npy'), x=label_x, y=label_y)

    inverse_deform(img_path, label, save_path)

    label_x = cv2.blur(label_x, (7, 7))
    label_y = cv2.blur(label_y, (7, 7))
    np.savez_compressed(os.path.join(file_save_folder, file_basename + '-prediction-blur.npy'), x=label_x, y=label_y)
    label = {
        'x': label_x,
        'y': label_y,
    }
