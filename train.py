...
def get_args():
    ...
    parser.add_argument('--data_path', default='/root/doc_unet_dataset/data_gen', help='dataset path')
    parser.add_argument('--pre_trained', default=False, help='use pre trained model')
    parser.add_argument('--pre_trained_path', help='pre trained model path')
...
def train(model, batch_size, epoch, train_data: Dataset, optimizer, logger, save_path):
    ...
    for epoch_idx in range(epoch):
        ...
        for batch_idx, (inputs, label_x, label_y) in enumerate(data_loader):
            ...
            print(f'epoch {str(epoch_idx)}, process {train_sample_sum / len(train_data):.2f}, time {time.time() - start:.2f}, loss {train_acc_sum / train_sample_sum:.3f}')  # noqa
