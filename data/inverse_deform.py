...
def get_c_funcs() -> dict:
    lib = np.ctypeslib.load_library('inverse_deform', '/root/DocUNet/data/c_src')
...
def inverse_deform_padding(img_path, label: dict, funcs):
    ...
    offset = np.array(
        tuple(map(lambda x: 0.5 * x, src_shape))).astype(np.int32)
...
def crop(dst_img_b, dst_img_g, dst_img_r, mask, funcs):
    ...
    min_x, max_x, min_y, max_y = edge[0] + \
        1, edge[1] - 1, edge[2] + 1, edge[3] - 1
...
def inverse_deform(img_path, label, save_path):
    funcs = get_c_funcs()
    dst_img_b, dst_img_g, dst_img_r, mask = inverse_deform_padding(
        img_path, label, funcs)
    dst_img_b, dst_img_g, dst_img_r, mask = crop(
        dst_img_b, dst_img_g, dst_img_r, mask, funcs)
    dst_img_b, dst_img_g, dst_img_r = interpolate(
        dst_img_b, dst_img_g, dst_img_r, mask, funcs)
...
if __name__ == '__main__':
    img_path = '/root/data_generate/image/229-5574a4a0.png'
    label_path = '/root/DocUNet/model/test.npy'
    save_path = '/root/DocUNet/model/test.png'
