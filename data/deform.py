...
# from data.anti_deform import from_label_deform
# from data.cover_texture import
...
def get_random_vs(rows, cols):
    ...
    vertex = (random.randint(int(0.25 * rows), int(0.75 * rows)),
              random.randint(int(0.25 * cols), int(0.75 * cols)))
...
def deform(label_shape, label_x, label_y, type):
    ...
    # prepare data
    new_label_x, new_label_y = np.zeros(label_shape).astype(
        np.float64), np.zeros(label_shape).astype(np.float64)
    ...
    # defrom
    c_deform(label_x, label_y, new_label_x,
             new_label_y, shape, vertex, v, type)
...
def gen_deform_label(img_path, resize_shape, operation: list):
    ...
    i_rows_start, i_rows_end, i_cols_start, i_cols_end = int(0.25 * label_shape[0]), \
        int(0.75 * label_shape[0]), int(0.25 *
                                        label_shape[1]), int(0.75 * label_shape[1])
